...
> **Icons:** placeholder icons are generated by `scripts/generate-icons.js` (solid indigo squares).
> Replace `images/icon-{16,48,128}.png` with your own artwork before running `pnpm build` if desired.
...
├── scripts/
│   ├── generate-icons.js   # One-time PNG icon generator (Node built-ins only)
